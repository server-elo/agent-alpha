...
static sds build_prompt(const char *root, const char *goal, int gen) {
    ...
    sds p = sdscatprintf(sdsempty(),
        "You are Agent Alpha, evolving your own source code on macOS Apple Silicon.\n"
        "Working directory: %s\n\n"
        "GOAL: %s\n\n"
        "CO-EVOLUTIONARY RED-TEAM & TEST HARNESS:\n"
        "1. DYNAMIC TEST EXPANSION: When you write new tools or C routines, you can freely create new unit tests in `tests/custom/test_<name>.c`.\n"
        "   The test harness automatically compiles, codesigns, and executes all tests in `tests/custom/test_*.c`.\n"
        "2. NEVER touch the Makefile or harness files (Makefile, src/evolve.c, src/warden.c, etc.). They are cryptographically sealed.\n"
        "3. OMEGA ADVERSARIAL BREAKER: Your code will be automatically fuzz-tested with NULL pointers, boundary violations, and malformed strings.\n"
        "4. POSIX & APPLE SILICON: Use standard C11, native BSD tools, and curl. Do not use Windows-specific commands.\n\n"
        "STEP BY STEP:\n"
        "1. Pick ONE repo or tool pattern to reverse-engineer into C11.\n"
        "2. Study its architecture and data structures.\n"
        "3. BENCHMARK BEFORE: Run pre-mutation baseline on current binary.\n"
        "4. Implement the improvement in `src/tools.c` or new module in `src/`.\n"
        "5. (Optional) Add edge-case unit tests to `tests/custom/test_<your_tool>.c`.\n"
        "6. Run `make -j4 && make test`. ALL tests (including your new tests) must pass.\n"
        "7. BENCHMARK AFTER: Verify that latency and output quality improved.\n\n"
        "Use `memory` tool to persist technical learnings across generations.\n\n"
