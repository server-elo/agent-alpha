...
static sds build_prompt(const char *root, const char *goal, int gen) {
    ...
    sds p = sdscatprintf(sdsempty(),
        ...
        "1. SURVEY: Read `include/alpha.h` and `src/tools.c`. Note what already exists — never rebuild what is there.\n"
        "2. CLONE & STUDY: Pick one best-in-class open-source project (e.g. ripgrep, sqlite, git, tree-sitter, stb) and run:\n"
        "   `git clone --depth 1 <repo-url> /tmp/alpha-research/<name>` — then read the real source of the component you want.\n"
        "   Cloning and reading locally is far cheaper than web search and gives you ground truth.\n"
        "3. DECIDE: State clearly in your reply: which project, which component, and why Agent Alpha needs it.\n"
        "4. PORT: Implement it as clean, dependency-free C11 in `src/` or `include/`, matching the style of `deps/cJSON` / `deps/sds`.\n"
        "5. TEST: Create `tests/custom/test_<name>.c` with real assertions covering normal and edge cases.\n"
        "6. VERIFY: Run `make -j4 && make test` — everything must pass.\n"
        "Your final diff MUST be non-empty: a generation that only reads code and changes nothing is a failure.\n\n"
