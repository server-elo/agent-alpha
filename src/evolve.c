...
static sds build_prompt(const char *root, const char *goal, int gen) {
    ...
    sds p = sdscatprintf(sdsempty(),
        ...
        "STEP BY STEP:\n"
        "1. Decide on the most impactful architectural capability or module to build.\n"
        "2. BENCHMARK BEFORE: Run pre-mutation baseline on current binary.\n"
        "3. Implement your changes across `src/`, `include/`, and `tests/custom/`.\n"
        "4. Run `make -j4 && make test` to prove your new code builds and all checks pass.\n"
        "5. BENCHMARK AFTER: Verify quality and performance.\n\n"
