...
static sds build_prompt(const char *root, const char *goal, int gen) {
    ...
    sds p = sdscatprintf(sdsempty(),
        ...
        "Working directory: %s\n\n"
        "GOAL: %s\n\n"
        "TOTAL ARCHITECTURAL SOVEREIGNTY:\n"
        "You have complete creative freedom to decide what to build, modify, or architect across the ENTIRE project.\n"
        "You are NOT restricted to any single file or directory. You can:\n"
        "- Architect brand-new C modules and subsystems anywhere in `src/`\n"
        "- Expand or redesign data structures and public interfaces in `include/alpha.h` or new headers\n"
        "- Build native tools, embedded databases, AST parsers, streaming memory, or network protocols\n"
        "- Upgrade the agent loop, Telegram engine, browser integration, or provider backends\n"
        "- Create rich custom unit tests in `tests/custom/test_<name>.c` to certify new capabilities\n"
        "- You decide the engineering direction, architecture, and features for maximum intelligence and speed.\n\n"
        ...
        "STEP BY STEP:\n"
        "1. INSPECT AGENT ALPHA: Read `include/alpha.h` and `src/tools.c` to survey existing tools, structs, and interfaces.\n"
        "2. LOOK UP & DECIDE: Choose the next capability to build (e.g., AST code parser, embedded KV store, diff-patch engine, regex search, streaming memory).\n"
        "3. BENCHMARK BEFORE: Run pre-mutation baseline on current binary.\n"
        "4. IMPLEMENT: Write your new C code across `src/` / `include/` and create rich unit tests in `tests/custom/test_<name>.c`.\n"
        "5. VERIFY: Run `make -j4 && make test` to prove your code compiles cleanly and all test suites pass.\n"
        "6. BENCHMARK AFTER: Verify that latency and output quality improved.\n\n"
