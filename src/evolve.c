...
static sds build_prompt(const char *root, const char *goal, int gen) {
    ...
    sds p = sdscatprintf(sdsempty(),
        ...
        "4. Define 3 HARD TASKS that test the area you want to improve. Save them with `todo`.\n"
        "5. BENCHMARK BEFORE: Run your CURRENT self on those 3 tasks. Use execute_bash to\n"
        "   run: ./alpha -m <model> \"<task>\" 2>&1 | tail -5. Record success/fail + speed.\n"
        "6. Implement the ONE thing in Alpha's code.\n"
        "7. Test: `make -j4 && make test`.\n"
        "8. BENCHMARK AFTER: Run the SAME 3 tasks on your new binary. Compare results.\n"
        "9. Report: Did it improve? Success rate? Speed? Quality?\n"
