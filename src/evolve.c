...
/* mkdir -p without a shell (same logic as tools.c's mkdir_p; kept local so
 * evolve.c stays self-contained). */
static int evolve_mkdir_p(const char *dir) {
...
/* A revert used to hard-delete every untracked file with `git clean -fd -q`,
 * so a false revert destroyed work products that were never committed. Move
 * them to evolution/quarantine/gen-NNN/ instead (evolution/ is git-ignored,
 * so the quarantine can never list or contain itself). */
static void quarantine_untracked(const char *root, int gen) {
    char probe[PATH_MAX];
    snprintf(probe, sizeof(probe), "%s/.git", root);
    if (!file_exists(probe)) return;

    int rc = -1;
    sds out = run_capture(root, "git ls-files --others --exclude-standard", 30, &rc);
    if (rc != 0 || !out || !out[0]) { sdsfree(out); return; }

    char qdir[PATH_MAX];
    snprintf(qdir, sizeof(qdir), "%s/evolution/quarantine/gen-%03d", root, gen);
    if (evolve_mkdir_p(qdir) != 0) { sdsfree(out); return; }
    ...
    for (char *line = strtok_r(copy, "\n", &saveptr); line;
        ...
        snprintf(src, sizeof(src), "%s/%s", root, line);
        snprintf(dst, sizeof(dst), "%s/%s", qdir, line);
        char *slash = strrchr(dst, '/');
        if (slash) { *slash = 0; evolve_mkdir_p(dst); *slash = '/'; }
        rename(src, dst);
    }
    sdsfree(copy);
    sdsfree(out);
...
static int next_generation(const char *root) {
    ...
    buf[rd] = 0;
    int max = 0;
    /* "gen": counts only at the start of a log line — that is the log format
     * (one JSON object per line, gen first). A note quoting the same substring
     * mid-line must not inflate the counter. */
    for (char *p = buf; p && *p; ) {
        if (strncmp(p, "{\"gen\":", 7) == 0) {
            int g = atoi(p + 7);
            if (g > max) max = g;
        }
        p = strchr(p, '\n');
...
int evolve_run(alpha_cfg_t *cfg, const char *goal, int generations, int reexec) {
    ...
    for (int g = 0; g < generations; g++) {
        ...
        if (ok) {
             ...
             * The report contains model-influenced text — shell_escape it before
             * it goes anywhere near sh -c. */
            sds safe_report = shell_escape(report ? report : "no report");
            sds snapcmd = sdscatprintf(sdsempty(),
                "rm -rf /tmp/alpha_revert_sandbox; cp -R '%s' /tmp/alpha_revert_sandbox 2>/dev/null; "
                "{ echo 'GEN=%d'; echo REPORT=%s; } > /tmp/alpha_revert_sandbox/.revert_report.txt; true",
                sandbox, gen, safe_report);
            sdsfree(safe_report);
            int snap_rc = -1;
            ...
            sdsfree(r);
            /* Untracked files are quarantined, never deleted (was: git clean -fd). */
            quarantine_untracked(root, gen);
