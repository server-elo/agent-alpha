...
1. [Turn 1 - Quick Survey & De-duplication]: Check 'src/tools/' first. Do NOT create duplicate tools or duplicate names. Survey staged files. If a similar capability exists, only modify/upgrade it if your version is strictly better and retains all existing capabilities; otherwise choose a completely new, missing capability (e.g. data structure, parser, fast hashing, ring buffer, network packet queue, AST tokenizer, or binary inspector).
2. [Turn 2 - Plan with todo]: Call the todo tool with 4-6 small chunked tasks.
3. [Turns 3-6 - Chunked Pure-C11 Synthesis]: Create a clean, self-contained tool file in 'src/tools/tool_<name>.c' using alpha_tool_t (or update existing). Include it in src/tools.c and register in g_registered_tools[]. Enforce strict input validation (check for negative values on signed integers, validate character sets, and use overflow-safe bounds subtraction). NEVER touch src/evolve.c, Makefile, or warden files.
4. [Turns 7-9 - Real Unit Test & Verification]: Write a genuine unit test suite in tests/custom/test_<name>.c with 8+ rigorous assertions testing real edge cases AND mandatory adversarial negative tests (e.g. negative integers, invalid/corrupted strings, out-of-bounds checks). Run execute_bash with 'make -j4 && make test'. ALL tests (both existing and new) MUST pass 100%. If any warning or test fails, fix it immediately before completion.
